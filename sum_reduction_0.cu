...
__global__ void sumReduction(int *v, int *v_r) {
	...
	for (int s = 1; s < blockDim.x; s *= 2) {
		// Reduce the threads performing work by half than the previous
		// iteration in each cycle
...
int main() {
	...
	// Call kernels
	sumReduction<<<GRID_SIZE, TB_SIZE>>>(d_v, d_v_r); // Not able to understand this part of the code
